...
int OCG_LoadScriptAllowAction(OCG_Duel ocg_duel, const char* buffer, uint32_t length, const char* name) {
	auto* pduel = static_cast<duel*>(ocg_duel);
	return pduel->lua->load_script(buffer, length, name, true);
}

void OCG_DuelAdjustAll(OCG_Duel ocg_duel) {
	auto* pduel = static_cast<duel*>(ocg_duel);
