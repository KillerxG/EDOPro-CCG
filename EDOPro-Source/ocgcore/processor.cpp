...
bool field::process(Processors::Turn& arg) {
	...
	case 3: {
		...
		if(is_flag(DUEL_1ST_TURN_DRAW) || (infos.turn_id > 1)) {
			...
			auto monster_count = [this](uint8_t playerid) {
				return std::count_if(player[playerid].list_mzone.begin(), player[playerid].list_mzone.end(), [](card* pcard) {
					return pcard != nullptr;
				});
			};
			if(count > 0 && is_flag(DUEL_DESTINY_DRAW) && !core.epsilon_destiny_draw_used[turn_player]
			   && player[turn_player].lp <= 3000 && !player[turn_player].list_main.empty()
			   && monster_count(1 - turn_player) > monster_count(turn_player)) {
				core.epsilon_draw_phase_destiny_draw = true;
				emplace_process<Processors::SelectYesNo>(turn_player, 1134);
				arg.step = 99;
				return FALSE;
			}
			if(count > 0) {
	...
	case 100: {
		if(returns.at<int32_t>(0) != 1) {
			core.epsilon_draw_phase_destiny_draw = false;
			int32_t count = get_draw_count(infos.turn_player);
			if(count > 0 && is_flag(DUEL_DRAW_UNTIL_5)) {
				count = std::max<int32_t>(static_cast<int32_t>(5 - player[turn_player].list_hand.size()), count);
			}
		...
		core.epsilon_destiny_draw_used[turn_player] = true;
		core.select_cards.assign(player[turn_player].list_main.begin(), player[turn_player].list_main.end());
		auto message = pduel->new_message(MSG_HINT);
		message->write<uint8_t>(HINT_SELECTMSG);
		message->write<uint8_t>(turn_player);
		message->write<uint64_t>(1134);
		emplace_process<Processors::SelectCard>(turn_player, false, 1, 1);
		return FALSE;
	}
	case 101: {
		...
		auto& deck = player[turn_player].list_main;
		if(auto it = std::find(deck.begin(), deck.end(), return_cards.list.front()); it != deck.end()) {
			auto selected = *it;
			deck.erase(it);
			deck.push_back(selected);
			reset_sequence(turn_player, LOCATION_DECK);
		}
		return FALSE;
	}
	case 102: {
		if(core.epsilon_draw_phase_destiny_draw) {
			core.epsilon_draw_phase_destiny_draw = false;
			arg.step = 2;
			return FALSE;
		}
...
bool field::process(Processors::Startup& arg) {
	...
	case 1: {
		...
		if(is_flag(DUEL_MULLIGAN)) {
			emplace_process<Processors::SelectYesNo>(0, 1664);
			return FALSE;
		}
	...
	case 2: {
		core.epsilon_mulligan_used[0] = returns.at<int32_t>(0) == 1 && !player[0].list_hand.empty();
		if(core.epsilon_mulligan_used[0])
			send_to(card_set{ player[0].list_hand.begin(), player[0].list_hand.end() }, nullptr, REASON_RULE, PLAYER_NONE, PLAYER_NONE, LOCATION_DECK, 2, POS_FACEDOWN);
		return FALSE;
	}
	case 3: {
		if(core.epsilon_mulligan_used[0]) {
			shuffle(0, LOCATION_DECK);
			draw(nullptr, REASON_RULE, PLAYER_NONE, 0, std::max(0, player[0].start_count - 1));
			return FALSE;
		}
		return FALSE;
	}
	case 4: {
		emplace_process<Processors::SelectYesNo>(1, 1664);
		return FALSE;
	}
	case 5: {
		core.epsilon_mulligan_used[1] = returns.at<int32_t>(0) == 1 && !player[1].list_hand.empty();
		if(core.epsilon_mulligan_used[1])
			send_to(card_set{ player[1].list_hand.begin(), player[1].list_hand.end() }, nullptr, REASON_RULE, PLAYER_NONE, PLAYER_NONE, LOCATION_DECK, 2, POS_FACEDOWN);
		return FALSE;
	}
	case 6: {
		if(core.epsilon_mulligan_used[1]) {
			shuffle(1, LOCATION_DECK);
			draw(nullptr, REASON_RULE, PLAYER_NONE, 1, std::max(0, player[1].start_count - 1));
			return FALSE;
		}
		return FALSE;
	}
