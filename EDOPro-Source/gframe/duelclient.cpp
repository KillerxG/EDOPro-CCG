...
static bool HasSkillCoverImage(uint32_t code) {
	for(auto& path : mainGame->cover_dirs) {
		for(auto extension : { EPRO_TEXT(".png"), EPRO_TEXT(".jpg") }) {
			if(path == EPRO_TEXT("archives")) {
				if(auto archive_file = Utils::FindFileInArchives(EPRO_TEXT("pics/cover/"), epro::format(EPRO_TEXT("{}{}"), code, extension))) {
					archive_file->drop();
					return true;
				}
...
#undef CHK
		...
		str.append(epro::format(L"{}{}\n", gDataManager->GetSysString(1225),
			pkt.info.rule == static_cast<uint8_t>(DuelAllowedCards::ALLOWED_CARDS_OFFICIAL_CCG) ? L"Official/CCG" : gDataManager->GetSysString(1899 + pkt.info.rule)));
		if(mainGame->dInfo.compat_mode)
		...
		if(!mainGame->dInfo.compat_mode) {
			for(int flag = SEALED_DUEL, i = 0; flag < MULLIGAN + 1; flag = flag << 1, i++)
		...
		mainGame->extra_rules = pkt.info.extra_rules;
		std::lock_guard<epro::mutex> lock(mainGame->gMutex);
...
inline bool PlayOST(uint32_t code) {
	if(!mainGame->dInfo.isCatchingUp)
		return gSoundManager->PlayOST(code);
	return true;
}
...
int DuelClient::ClientAnalyze(const uint8_t* msg, uint32_t len) {
	...
	case MSG_HINT: {
		...
		case HINT_SKILL_COVER: {
			...
			pcard->location = LOCATION_SKILL;
			skills.push_back(pcard);
			const auto skill_code = static_cast<uint32_t>((data >> 32) & 0xffffffff);
			const auto fallback_cover = static_cast<uint32_t>(data & 0xffffffff);
			pcard->cover = HasSkillCoverImage(skill_code) ? skill_code : fallback_cover;
			pcard->SetCode(skill_code);
			if(!mainGame->dInfo.isCatchingUp) {
				mainGame->dField.MoveCard(pcard, 10);
				mainGame->WaitFrameSignal(11, lock);
			}
		...
		case HINT_SKILL_FLIP: {
			...
			auto& skills = mainGame->dField.skill_cards[player];
			auto& pcard = mainGame->dField.skills[player];
			const auto code = data & 0xffffffff;
			for(auto& skill : skills) {
				if(skill && skill->code == code) {
					pcard = skill;
					break;
				}
			}
		...
		case HINT_SKILL_REMOVE: {
			...
			auto& pcard = mainGame->dField.skills[player];
			const auto code = data & 0xffffffff;
			ClientCard* removed_card = nullptr;
			...
			if(removed_card) {
				if(!mainGame->dInfo.isCatchingUp) {
					...
					if(gGameConfig->quick_animation)
						frames = 12;
					mainGame->dField.FadeCard(removed_card, 5, frames);
					mainGame->WaitFrameSignal(frames, lock);
				}
				if(removed_card == mainGame->dField.hovered_card)
					mainGame->dField.hovered_card = nullptr;
				delete removed_card;
				pcard = skills.empty() ? nullptr : skills.back();
			}
			break;
		}
		}
	...
	case MSG_SUMMONING: {
		const auto code = BufferIO::Read<uint32_t>(pbuf);
		/*CoreUtils::loc_info info = CoreUtils::ReadLocInfo(pbuf, mainGame->dInfo.compat_mode);*/
		if(!PlayOST(code) && !PlayChant(SoundManager::CHANT::SUMMON, code))
		...
		if(!mainGame->dInfo.isCatchingUp) {
			...
			mainGame->WaitFrameSignal(30, lock);
			mainGame->showcard = 0;
			mainGame->StartCardVideo(EPRO_TEXT("summon"), code);
			mainGame->WaitFrameSignal(11, lock);
		}
		return true;
	}
	...
	case MSG_SPSUMMONING: {
		const auto code = BufferIO::Read<uint32_t>(pbuf);
		/*CoreUtils::loc_info info = CoreUtils::ReadLocInfo(pbuf, mainGame->dInfo.compat_mode);*/
		if(!code || (!PlayOST(code) && !PlayChant(SoundManager::CHANT::SUMMON, code)))
	...
	case MSG_CHAINING: {
		const auto code = BufferIO::Read<uint32_t>(pbuf);
		if(!PlayOST(code) && !PlayChant(SoundManager::CHANT::ACTIVATE, code))
			Play(SoundManager::SFX::ACTIVATE);
		CoreUtils::loc_info info = CoreUtils::ReadLocInfo(pbuf, mainGame->dInfo.compat_mode);
		const auto cc = mainGame->LocalPlayer(BufferIO::Read<uint8_t>(pbuf));
		const auto cl = BufferIO::Read<uint8_t>(pbuf);
		...
		if(!mainGame->dInfo.isCatchingUp) {
			...
			if(pcard->location & 0x30) {
				...
				mainGame->WaitFrameSignal(30, lock);
			mainGame->StartCardVideo(EPRO_TEXT("activate"), code);
			pcard->is_highlighting = false;
		}
