...
struct host_creation_panel_elements {
	...
	irr::gui::IGUICheckBox* chkCustomRules[7+12+8+5+2];
...
struct deck_edit_page_elements {
	...
	irr::gui::IGUIButton* btnManualDuel;
	...
	irr::gui::IGUICheckBox* chkCustom;
	//sort type
...
public:
	...
	void DrawHolograms();
	void DrawHologram(ClientCard* pcard);
	struct CardVideoFrame;
	uint32_t StartCardVideo(epro::path_stringview video_type, uint32_t code);
	bool ReadCardVideoFrame(CardVideoFrame& frame);
	void DrawCardVideo();
	void WaitMilliseconds(uint32_t milliseconds, std::unique_lock<epro::mutex>& _lck);
	void DrawMisc();
	...
	struct CardVideoFrame {
		...
		epro::path_string image_path;
		uint32_t timestamp_ms;
	};
	bool card_video_active{ false };
	std::vector<CardVideoFrame> card_video_frames;
	irr::video::ITexture* card_video_texture{ nullptr };
	void* card_video_reader{ nullptr };
	int64_t card_video_first_timestamp{ -1 };
	uint32_t card_video_source_width{ 0 };
	uint32_t card_video_source_height{ 0 };
	uint32_t card_video_width{ 0 };
	uint32_t card_video_height{ 0 };
	uint32_t card_video_elapsed{ 0 };
	uint32_t card_video_duration{ 0 };
	uint32_t card_video_frame_ms{ 33 };
	uint32_t card_video_next_frame_ms{ 33 };
	bool card_video_finished{ false };
	size_t card_video_texture_frame{ static_cast<size_t>(-1) };
	uint32_t card_video_texture_serial{ 0 };
	bool is_attacking;
	float attack_sv;
