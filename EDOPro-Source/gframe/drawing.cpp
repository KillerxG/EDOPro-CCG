...
bool EnsureMediaFoundationStarted() {
	...
	if(failed)
		return false;
	const auto cohr = CoInitializeEx(nullptr, COINIT_MULTITHREADED);
...
void Game::DrawHolograms() {
	if(!gGameConfig->draw_holograms)
		return;
	for(int p = 0; p < 2; ++p) {
		for(auto& pcard : dField.mzone[p]) {
			if(pcard)
...
void Game::DrawHologram(ClientCard* pcard) {
	if(pcard->is_moving || !(pcard->location & LOCATION_MZONE) || !(pcard->position & POS_FACEUP))
		return;
	static std::unordered_map<uint32_t, irr::video::ITexture*> hologram_textures;
	auto [it, inserted] = hologram_textures.try_emplace(pcard->code, nullptr);
	if(inserted) {
		auto load_hologram = [this](epro::path_stringview path) {
			auto* img = driver->createImageFromFile({ path.data(), static_cast<irr::u32>(path.size()) });
			if(!img)
				return static_cast<irr::video::ITexture*>(nullptr);
			const auto& dim = img->getDimension();
			for(irr::u32 y = 0; y < dim.Height; ++y) {
				for(irr::u32 x = 0; x < dim.Width; ++x) {
			...
			auto* texture = driver->addTexture({ path.data(), static_cast<irr::u32>(path.size()) }, img);
			img->drop();
			return texture;
		};
		auto path = FindRepositoryAnimationFile(EPRO_TEXT("holograms"), pcard->code, { EPRO_TEXT(".png"sv), EPRO_TEXT(".jpg"sv) });
		if(path.empty())
			path = epro::format(EPRO_TEXT("./Custom features/holograms/{}.png"), pcard->code);
		it->second = load_hologram(path);
		if(!it->second) {
			path = epro::format(EPRO_TEXT("./Custom features/holograms/{}.jpg"), pcard->code);
			it->second = driver->getTexture(path.data());
		}
	}
	...
	const auto float_offset = std::sin(static_cast<float>(ms % 3000) / 3000.0f * irr::core::PI * 2.0f) * 0.035f;
	irr::video::SMaterial hologram_material;
	hologram_material.AmbientColor = 0xffffffff;
	hologram_material.DiffuseColor = 0xffffffff;
	hologram_material.ColorMaterial = irr::video::ECM_NONE;
	hologram_material.MaterialType = irr::video::EMT_TRANSPARENT_ALPHA_CHANNEL_REF;
	hologram_material.setFlag(irr::video::EMF_BACK_FACE_CULLING, false);
#if (IRRLICHT_VERSION_MAJOR==1 && IRRLICHT_VERSION_MINOR==9)
	hologram_material.ZWriteEnable = irr::video::EZW_OFF;
#endif
	hologram_material.setTexture(0, texture);
	driver->setMaterial(hologram_material);
	irr::core::matrix4 hologram;
	hologram.setScale(irr::core::vector3df(1.52f, 1.52f, 1.52f));
	hologram.setRotationRadians(pcard->curRot);
	hologram.setTranslation(pcard->curPos + irr::core::vector3df(0.0f, 0.0f, 0.22f + float_offset));
	driver->setTransform(irr::video::ETS_WORLD, hologram);
	driver->drawVertexPrimitiveList(matManager.vCardFront, 4, matManager.iRectangle, 2);
}
uint32_t Game::StartCardVideo(epro::path_stringview video_type, uint32_t code) {
	if(!gGameConfig->draw_card_animations)
		return 0;
	if(!code)
		return 0;
	auto path = FindRepositoryAnimationFile(video_type, code, { EPRO_TEXT(".mp4"sv) });
	if(path.empty())
		path = epro::format(EPRO_TEXT("./Custom features/summon_videos/{}/{}.mp4"), video_type, code);
	card_video_active = false;
	if(card_video_texture) {
		driver->removeTexture(card_video_texture);
		card_video_texture = nullptr;
	}
	card_video_texture_frame = static_cast<size_t>(-1);
	card_video_frames.clear();
	card_video_frames.shrink_to_fit();
#if EDOPRO_WINDOWS
	if(card_video_reader) {
		auto* old_reader = static_cast<IMFSourceReader*>(card_video_reader);
		SafeRelease(old_reader);
		card_video_reader = nullptr;
	}
#endif
	card_video_frame_ms = 33;
	auto try_load_frame_sequence = [&](uint32_t first_index) {
		for(uint32_t index = first_index; index < first_index + 3000; ++index) {
			auto frame_path = epro::format(EPRO_TEXT("./Custom features/summon_videos/{}/{}/{:04}.png"), video_type, code, index);
			if(!Utils::FileExists(frame_path)) {
				frame_path = epro::format(EPRO_TEXT("./Custom features/summon_videos/{}/{}/{:04}.jpg"), video_type, code, index);
				if(!Utils::FileExists(frame_path)) {
					frame_path = FindRepositoryAnimationFrameFile(video_type, code, index);
					if(frame_path.empty())
						break;
				}
			}
			CardVideoFrame frame;
			frame.image_path = std::move(frame_path);
			frame.timestamp_ms = (index - first_index) * card_video_frame_ms;
			card_video_frames.push_back(std::move(frame));
		}
		return !card_video_frames.empty();
	};
	if(try_load_frame_sequence(0) || try_load_frame_sequence(1)) {
		card_video_elapsed = 0;
		card_video_duration = card_video_frames.size() == 1 ? 3000 : card_video_frames.back().timestamp_ms + card_video_frame_ms;
		card_video_finished = true;
		card_video_texture_frame = static_cast<size_t>(-1);
		++card_video_texture_serial;
		card_video_active = true;
		ErrorLog("[RPG_VIDEO] Started frame sequence for {} {} ({} frames, {} ms)", Utils::ToUTF8IfNeeded(video_type), code, card_video_frames.size(), card_video_duration);
		return card_video_duration;
	}
	auto still_path = FindRepositoryAnimationFile(video_type, code, { EPRO_TEXT(".png"sv), EPRO_TEXT(".jpg"sv) });
	if(!still_path.empty()) {
		CardVideoFrame frame;
		frame.image_path = std::move(still_path);
		frame.timestamp_ms = 0;
		card_video_frames.push_back(std::move(frame));
		card_video_elapsed = 0;
		card_video_duration = 3000;
		card_video_finished = true;
		card_video_texture_frame = static_cast<size_t>(-1);
		++card_video_texture_serial;
		card_video_active = true;
		ErrorLog("[RPG_VIDEO] Started still animation for {} {}", Utils::ToUTF8IfNeeded(video_type), code);
		return card_video_duration;
	}
	if(!Utils::FileExists(path))
		return 0;
	...
	if(!EnsureMediaFoundationStarted()) {
		ErrorLog("[RPG_VIDEO] Media Foundation failed to start");
		return 0;
	}
	ErrorLog("[RPG_VIDEO] Loading {}", Utils::ToUTF8IfNeeded(path));
	...
	if(SUCCEEDED(hr))
		hr = MFCreateSourceReaderFromURL(path.data(), attributes, &reader);
	...
	if(SUCCEEDED(hr))
		hr = reader->SetCurrentMediaType(video_stream, nullptr, media_type);
	if(SUCCEEDED(hr))
		hr = reader->GetCurrentMediaType(video_stream, &current_type);
	UINT32 width = 0;
	UINT32 height = 0;
	UINT32 fps_num = 30;
	UINT32 fps_den = 1;
	...
	if(SUCCEEDED(hr))
		MFGetAttributeRatio(current_type, MF_MT_FRAME_RATE, &fps_num, &fps_den);
	if(FAILED(hr) || !width || !height) {
		ErrorLog("[RPG_VIDEO] Failed to prepare {}: 0x{:08x}", Utils::ToUTF8IfNeeded(path), static_cast<uint32_t>(hr));
		SafeRelease(current_type);
		SafeRelease(media_type);
		SafeRelease(reader);
		return 0;
	}
	constexpr UINT32 max_video_dimension = 1280;
	const UINT32 source_width = width;
	const UINT32 source_height = height;
	UINT32 target_width = source_width;
	UINT32 target_height = source_height;
	const auto largest_dimension = std::max(source_width, source_height);
	if(largest_dimension > max_video_dimension) {
		target_width = std::max<UINT32>(1, (static_cast<uint64_t>(source_width) * max_video_dimension) / largest_dimension);
		target_height = std::max<UINT32>(1, (static_cast<uint64_t>(source_height) * max_video_dimension) / largest_dimension);
	}
	SafeRelease(current_type);
	SafeRelease(media_type);
	card_video_reader = reader;
	reader = nullptr;
	card_video_first_timestamp = -1;
	card_video_source_width = source_width;
	card_video_source_height = source_height;
	card_video_width = target_width;
	card_video_height = target_height;
	card_video_elapsed = 0;
	auto frame_ms = (fps_num && fps_den) ? static_cast<uint32_t>((1000ull * fps_den) / fps_num) : 33u;
	if(!frame_ms)
		frame_ms = 33;
	card_video_frame_ms = frame_ms;
	card_video_next_frame_ms = frame_ms;
	card_video_duration = 0;
	card_video_finished = false;
	CardVideoFrame first_frame;
	if(!ReadCardVideoFrame(first_frame)) {
		ErrorLog("[RPG_VIDEO] No first frame decoded from {}", Utils::ToUTF8IfNeeded(path));
		auto* retained_reader = static_cast<IMFSourceReader*>(card_video_reader);
		SafeRelease(retained_reader);
		card_video_reader = nullptr;
		SafeRelease(reader);
		return 0;
	}
	uint64_t first_frame_sum = 0;
	for(size_t i = 0; i + 2 < first_frame.pixels.size(); i += 4)
		first_frame_sum += first_frame.pixels[i] + first_frame.pixels[i + 1] + first_frame.pixels[i + 2];
	const auto first_frame_avg = first_frame_sum / std::max<size_t>(1, first_frame.pixels.size() / 4 * 3);
	card_video_frames.push_back(std::move(first_frame));
	if(card_video_texture) {
		driver->removeTexture(card_video_texture);
		card_video_texture = nullptr;
	}
	...
	++card_video_texture_serial;
	card_video_active = true;
	if(card_video_active)
		ErrorLog("[RPG_VIDEO] Started stream from {} ({}x{}, first avg {})", Utils::ToUTF8IfNeeded(path), card_video_width, card_video_height, first_frame_avg);
	else
		ErrorLog("[RPG_VIDEO] Failed creating texture for {}", Utils::ToUTF8IfNeeded(path));
	return card_video_active ? 1 : 0;
#else
	return 0;
#endif
}
bool Game::ReadCardVideoFrame(CardVideoFrame& frame) {
#if EDOPRO_WINDOWS
	auto* reader = static_cast<IMFSourceReader*>(card_video_reader);
	if(!reader || !card_video_source_width || !card_video_source_height || !card_video_width || !card_video_height)
		return false;
	constexpr DWORD video_stream = static_cast<DWORD>(MF_SOURCE_READER_FIRST_VIDEO_STREAM);
	...
	while(true) {
		...
		auto hr = reader->ReadSample(video_stream, 0, &stream_index, &flags, &timestamp, &sample);
		...
		if(SUCCEEDED(hr)) {
			...
			if(SUCCEEDED(hr)) {
				const auto wanted = static_cast<size_t>(card_video_source_width) * static_cast<size_t>(card_video_source_height) * source_pixel_size;
				if(current_length >= wanted) {
					if(card_video_first_timestamp < 0)
						card_video_first_timestamp = timestamp;
					frame.timestamp_ms = static_cast<uint32_t>((timestamp - card_video_first_timestamp) / 10000);
					frame.pixels.resize(static_cast<size_t>(card_video_width) * static_cast<size_t>(card_video_height) * source_pixel_size);
					for(uint32_t y = 0; y < card_video_height; ++y) {
						const auto source_y = (static_cast<uint64_t>(y) * card_video_source_height) / card_video_height;
						const auto* source_row = data + static_cast<size_t>(source_y) * card_video_source_width * source_pixel_size;
						auto* target_row = frame.pixels.data() + static_cast<size_t>(y) * card_video_width * source_pixel_size;
						for(uint32_t x = 0; x < card_video_width; ++x) {
							const auto source_x = (static_cast<uint64_t>(x) * card_video_source_width) / card_video_width;
							memcpy(target_row + static_cast<size_t>(x) * source_pixel_size, source_row + static_cast<size_t>(source_x) * source_pixel_size, source_pixel_size);
							target_row[static_cast<size_t>(x) * source_pixel_size + 3] = 0xff;
						}
					}
					buffer->Unlock();
					SafeRelease(buffer);
...
void Game::DrawCardVideo() {
	if(!card_video_active || card_video_frames.empty())
		return;
	card_video_elapsed += delta_time;
	if(card_video_duration && card_video_elapsed >= card_video_duration) {
		card_video_active = false;
		if(card_video_texture) {
			driver->removeTexture(card_video_texture);
			card_video_texture = nullptr;
	...
	while(!card_video_finished && card_video_elapsed >= card_video_next_frame_ms) {
		CardVideoFrame next_frame;
		if(ReadCardVideoFrame(next_frame)) {
			card_video_frames[0] = std::move(next_frame);
			card_video_texture_frame = static_cast<size_t>(-1);
			card_video_next_frame_ms += card_video_frame_ms;
		} else {
			card_video_finished = true;
#if EDOPRO_WINDOWS
			if(card_video_reader) {
...
#endif
			card_video_duration = card_video_elapsed + card_video_frame_ms;
			break;
		}
	}
	size_t frame_index = 0;
	while(frame_index + 1 < card_video_frames.size() && card_video_frames[frame_index + 1].timestamp_ms <= card_video_elapsed)
		++frame_index;
	if(card_video_texture_frame != frame_index) {
		if(card_video_texture) {
			driver->removeTexture(card_video_texture);
			card_video_texture = nullptr;
		}
		auto& frame = card_video_frames[frame_index];
		if(!frame.image_path.empty()) {
			card_video_texture = driver->getTexture({ frame.image_path.data(), static_cast<irr::u32>(frame.image_path.size()) });
		} else {
			auto* image = driver->createImageFromData(
				irr::video::ECF_A8R8G8B8,
				irr::core::dimension2du(card_video_width, card_video_height),
				frame.pixels.data(),
				false);
			if(image) {
				const auto texture_name = epro::format(EPRO_TEXT("rpg_card_video_{}_{}"), card_video_texture_serial, frame_index);
				card_video_texture = driver->addTexture(texture_name.data(), image);
				image->drop();
			}
		}
		card_video_texture_frame = frame_index;
	}
	if(!card_video_texture)
		return;
	const auto img_size = card_video_texture->getOriginalSize();
	float scale = std::min(
		static_cast<float>(window_size.Width) / static_cast<float>(img_size.Width),
		static_cast<float>(window_size.Height) / static_cast<float>(img_size.Height));
	float still_offset = 0.0f;
	if(card_video_frames.size() == 1) {
		const auto progress = std::min(1.0f, static_cast<float>(card_video_elapsed) / static_cast<float>(std::max<uint32_t>(1, card_video_duration)));
		const auto eased_progress = 1.0f - ((1.0f - progress) * (1.0f - progress));
		scale *= 0.90f + eased_progress * 0.16f;
	}
	const auto draw_width = static_cast<irr::s32>(img_size.Width * scale);
	const auto draw_height = static_cast<irr::s32>(img_size.Height * scale);
	const irr::s32 x = (static_cast<irr::s32>(window_size.Width) - draw_width) / 2;
	const irr::s32 y = (static_cast<irr::s32>(window_size.Height) - draw_height) / 2 + static_cast<irr::s32>(still_offset);
	driver->draw2DRectangle(0xff000000, irr::core::recti(0, 0, window_size.Width, window_size.Height));
	driver->draw2DImage(card_video_texture,
		irr::core::recti(x, y, x + draw_width, y + draw_height),
		irr::core::recti(0, 0, img_size.Width, img_size.Height), nullptr, nullptr, false);
}
...
void Game::WaitMilliseconds(uint32_t milliseconds, std::unique_lock<epro::mutex>& _lck) {
	signalFrame = milliseconds;
	frameSignal.Wait(_lck);
}
...
void Game::DrawDeckBd() {
	...
	const auto CountGenesysPoints = [&](const Deck::Vector& cards)->uint32_t {
		uint32_t total = 0;
		if(!deckBuilder.filterList || !deckBuilder.filterList->genesys)
			return total;
		for(const auto* card : cards)
			total += static_cast<uint32_t>(deckBuilder.filterList->GetGenesysPoints(card));
		return total;
	};
	const bool use_genesys = deckBuilder.filterList && deckBuilder.filterList->genesys;
	...
	//main deck
	{
		...
		if(use_genesys) {
			const auto genesys_points = CountGenesysPoints(current_deck.main) + CountGenesysPoints(current_deck.extra) + CountGenesysPoints(current_deck.side);
			const auto color = genesys_points > deckBuilder.filterList->genesys_point_cap ? 0xffff5555 : 0xff66ff99;
			const auto genesys_text = epro::format(L"Genesys: {} / {}", genesys_points, deckBuilder.filterList->genesys_point_cap);
			DrawShadowText(textFont, genesys_text, Resize(450, 137, 610, 157), Resize(1, 1, 1, 1), color, 0xff000000, false, true);
		}
	...
	//search result
	{
		...
		for(; i < 9 && (i + card_position) < (int)deckBuilder.results.size(); ++i) {
			...
			if(use_genesys) {
				const auto point_text = epro::format(L"{} pts", deckBuilder.filterList->GetGenesysPoints(ptr));
				DrawShadowTextPos(textFont, point_text, Resize(958, height_offset + 208 + i * 66, 1018, height_offset + 229 + i * 66),
								  Resize(959, height_offset + 209 + i * 66, 1018, height_offset + 229 + i * 66), 0xff66ff99, 0xff000000, false, false, &rect);
			}
		}
	}
