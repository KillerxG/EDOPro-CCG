...
void SoundManager::RefreshOSTList() {
	if(!IsUsable())
		return;
	OSTList.clear();
...
bool SoundManager::PlayOST(uint32_t code) {
	if(!IsUsable())
		return false;
	if(!soundsEnabled)
		return false;
	auto ost_it = OSTList.find(code);
	if(ost_it == OSTList.end())
		return false;
	return mixer->PlaySound(ost_it->second);
}
