...
void ServerLobby::FillOnlineRooms() {
	...
	for(auto& room : rooms) {
		...
		roomListTable->setCellText(index, 1,
			room.info.rule == static_cast<uint8_t>(DuelAllowedCards::ALLOWED_CARDS_OFFICIAL_CCG) ? L"Official/CCG" : gDataManager->GetSysString(room.info.rule + 1899).data());
