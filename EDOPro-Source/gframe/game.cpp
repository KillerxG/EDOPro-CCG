...
static std::wstring FormatSkillInfoRace(const CardDataC* cd) {
	auto sys = [](uint32_t id) {
		const auto str = gDataManager->GetSysString(id);
		return std::wstring{ str.data(), str.size() };
	};
	if(cd->race)
		return gDataManager->FormatRace(cd->race, true);
	if(cd->ot & SCOPE_SPEED)
		return sys(1910);
	if(cd->ot & SCOPE_RUSH)
		return sys(1911);
	if((cd->ot & SCOPE_OCG_TCG) == SCOPE_OCG_TCG)
		return sys(1902);
	if(cd->ot & SCOPE_OCG)
		return sys(1900);
	if(cd->ot & SCOPE_TCG)
		return sys(1901);
	if(cd->ot & SCOPE_CUSTOM)
		return L"CCG";
	if(cd->ot & SCOPE_PRERELEASE)
		return sys(1903);
	return L"CCG";
}
...
#undef OFFSET
	...
	btnManualDuel = AlignElementWithParent(env->addButton(Scale(300, 90, 390, 130), nullptr, BUTTON_MANUAL_DUEL, L"Manual"));
	btnManualDuel->setVisible(false);
	...
	chkAnime = AlignElementWithParent(env->addCheckBox(gGameConfig->chkAnime, Scale(10, 84, 150, 106), wFilter, CHECKBOX_SHOW_ANIME, gDataManager->GetSysString(1999).data()));
	defaultStrings.emplace_back(chkAnime, 1999);
	chkCustom = AlignElementWithParent(env->addCheckBox(gGameConfig->chkCustom, Scale(10, 104, 150, 126), wFilter, CHECKBOX_SHOW_CUSTOM, L"Custom"));
	...
	btnExportDeck->setVisible(false);
...
bool Game::LoadCore() {
	...
	const auto running_mod_exe = Utils::GetFileName(Utils::GetExePath(), true) == EPRO_TEXT("EDOProMod.exe");
	if(running_mod_exe) {
...
#define WStr(i) epro::to_wstring<int>(i).data()
		...
		extra_rules = gGameConfig->lastExtraRules & ((MULLIGAN << 1) - 1);
...
void Game::PopulateSettingsWindow() {
...

	{
		...
		gSettings.chkDrawHolograms = env->addCheckBox(gGameConfig->draw_holograms, GetNextRect(), sPanel, CHECKBOX_DRAW_HOLOGRAMS, L"Hologram");
		gSettings.chkDrawCardAnimations = env->addCheckBox(gGameConfig->draw_card_animations, GetNextRect(), sPanel, CHECKBOX_DRAW_CARD_ANIMATIONS, L"Summon/Activation Animation");
		{
...
void Game::SaveConfig() {
	...
	gGameConfig->draw_holograms = gSettings.chkDrawHolograms->isChecked();
	gGameConfig->draw_card_animations = gSettings.chkDrawCardAnimations->isChecked();
	gGameConfig->chkAnime = chkAnime->isChecked();
	gGameConfig->chkCustom = chkCustom->isChecked();
#ifdef UPDATE_URL
...
void Game::UpdateRepoInfo(const GitRepo* repo, RepoGui* grepo) {
	...
	if(!repo->is_language) {
		...
		auto pics_path = Utils::ToPathString(repo->pics_path);
		pic_dirs.insert(pic_dirs.begin(), pics_path);
		cover_dirs.insert(cover_dirs.begin(), pics_path + EPRO_TEXT("cover/"));
		field_dirs.insert(field_dirs.begin(), pics_path + EPRO_TEXT("field/"));
		const auto running_mod_exe = Utils::GetFileName(Utils::GetExePath(), true) == EPRO_TEXT("EDOProMod.exe");
		if(repo->has_core && !running_mod_exe)
			cores_to_load.insert(cores_to_load.begin(), Utils::ToPathString(repo->core_path));
		auto lflist_path = Utils::NormalizePath(Utils::ToPathString(repo->lflist_path));
		auto lflist_prefix = BufferIO::DecodeUTF8(repo->lflist_prefix);
		const auto official_lflists = Utils::NormalizePath(EPRO_TEXT("./repositories/lflists/"));
		const auto ccg_lflists = Utils::NormalizePath(EPRO_TEXT("./repositories/lflistsCCG/"));
		bool loaded_lflist = false;
		if(lflist_path == official_lflists || lflist_path == ccg_lflists)
			loaded_lflist = gdeckManager->LoadLFListFolder(lflist_path, lflist_prefix);
		if(loaded_lflist) {
...
void Game::ShowCardInfo(uint32_t code, bool resize, imgType type) {
	...
	auto passcode_scope = epro::format(L"[{:08}] {}", tmp_code, gDataManager->FormatScope(cd->ot));
	if(is_building && deckBuilder.filterList && deckBuilder.filterList->genesys)
		passcode_scope.append(epro::format(L"   Genesys Points: {}", deckBuilder.filterList->GetGenesysPoints(cd)));
	stPasscodeScope->setText(passcode_scope.data());
	stSetName->setText(L"");
	...
	if(cd->type & TYPE_MONSTER) {
		...
		if(cd->type & TYPE_SKILL) { // TYPE_SKILL created by hints
			...
			stInfo->setText(epro::format(L"[{}|{}]", FormatSkillInfoRace(cd), gDataManager->FormatType(cd->type)).data());
		} else {
...
void Game::ReloadCBLimit() {
	...
	if(!white) {
		...
		if(chkAnime->isChecked() || chkCustom->isChecked())
...
void Game::ReloadCBFilterRule() {
	...
	cbFilterRule->addItem(L"Official/CCG");
...
OCG_Duel Game::SetupDuel(OCG_DuelOptions opts) {
	...
	LoadScript(pduel, "mod_globals.lua");
