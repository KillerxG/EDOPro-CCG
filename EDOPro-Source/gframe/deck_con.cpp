...
void DeckBuilder::Initialize(bool refresh) {
	...
	mainGame->btnManualDuel->setVisible(false);
...
bool DeckBuilder::OnEvent(const irr::SEvent& event) {
	...
	case irr::EET_GUI_EVENT: {
		...
		case irr::gui::EGET_BUTTON_CLICKED: {
			switch(id) {
			case BUTTON_MANUAL_DUEL: {
				Terminate(false);
				ManualMode::StartLocal();
				break;
			}
...
#undef CHECK
		...
		case irr::gui::EGET_COMBO_BOX_CHANGED: {
			...
			case COMBOBOX_DBLFLIST: {
				...
				mainGame->ShowCardInfo(mainGame->showingcard, true);
				StartFilter(true);
				break;
			}
			case COMBOBOX_DBDECKS: {
		...
		case irr::gui::EGET_CHECKBOX_CHANGED: {
			switch (id) {
				...
				case CHECKBOX_SHOW_CUSTOM: {
					int prevLimit = mainGame->cbLimit->getSelected();
					mainGame->ReloadCBLimit();
					if (prevLimit < 8 || prevLimit == LIMITATION_FILTER_CUSTOM)
						mainGame->cbLimit->setSelected(prevLimit);
					StartFilter(true);
					break;
				}
...
bool DeckBuilder::CheckCardProperties(const CardDataM& data) {
	if(data._data.type & TYPE_TOKEN || data._data.ot & SCOPE_HIDDEN)
		return false;
	if(mainGame->chkCustom->isChecked() && !filterList->whitelist) {
		if(data._data.ot & ~(SCOPE_OCG | SCOPE_TCG | SCOPE_CUSTOM))
			return false;
		if(!(data._data.ot & (SCOPE_OCG | SCOPE_TCG | SCOPE_CUSTOM)))
			return false;
	} else if((data._data.ot & SCOPE_OFFICIAL) != data._data.ot && (!mainGame->chkAnime->isChecked() && !filterList->whitelist))
		return false;
	switch(filter_type) {
	case 1: {
...
bool DeckBuilder::push_main(const CardDataC* pointer, int seq, bool forced) {
	...
	if(!forced && !mainGame->is_siding) {
		...
		if(main_legend_count_trap >= 1 && (pointer->ot & SCOPE_LEGEND) && (pointer->type & TYPE_TRAP))
			return false;
		if(main_skill_count >= 2 && (pointer->type & TYPE_SKILL))
			return false;
