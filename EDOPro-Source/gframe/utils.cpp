...
#if EDOPRO_WINDOWS
		SetFileAttributes(source.data(), FILE_ATTRIBUTE_NORMAL);
...
#define ROOT_OR_CUR(str) (str == EPRO_TEXT(".") || (str == EPRO_TEXT("..")))
	...
	bool Utils::DeleteDirectory(epro::path_stringview path) {
		ClearDirectory(path);
#if EDOPRO_WINDOWS
		SetFileAttributes(path.data(), FILE_ATTRIBUTE_NORMAL);
...
#if EDOPRO_WINDOWS
		if(Utils::GetFileName(Utils::GetExePath(), true) == EPRO_TEXT("EDOProMod.exe"))
			return epro::format(EPRO_TEXT("{}/expansions/ocgmod.dll"), Utils::GetExeFolder());
