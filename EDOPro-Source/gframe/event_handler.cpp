...
bool ClientField::OnEvent(const irr::SEvent& event) {
		...
		case irr::gui::EGET_BUTTON_CLICKED: {
			if(ManualMode::IsActive()) {
				...
				case BUTTON_CMD_RESET:
					return ManualMode::HandleCommand(*this, id);
				default:
					break;
				}
			}
	...
	case irr::EET_MOUSE_INPUT_EVENT: {
		...
		case irr::EMIE_LMOUSE_DOUBLE_CLICK: {
			...
			if((hovered_location & (LOCATION_DECK | LOCATION_GRAVE | LOCATION_REMOVED | LOCATION_EXTRA | LOCATION_SKILL)) == 0)
				break;
		...
		case irr::EMIE_LMOUSE_LEFT_UP: {
			...
			if(ManualMode::IsActive()) {
				ManualMode::ShowContextMenu(*this, x, y);
				break;
			}
			if(mainGame->dInfo.isReplay) {
				if(mainGame->wCardSelect->isVisible())
				...
				case LOCATION_SKILL: {
					if(skill_cards[hovered_controler].size() == 0)
						break;
					for(int32_t i = (int32_t)skill_cards[hovered_controler].size() - 1; i >= 0 ; --i)
						selectable_cards.push_back(skill_cards[hovered_controler][i]);
					mainGame->wCardSelect->setText(epro::format(L"{}({})", gDataManager->GetSysString(1077), skill_cards[hovered_controler].size()).data());
					break;
				}
				}
...
bool ClientField::OnCommonEvent(const irr::SEvent& event, bool& stopPropagation) {
	...
	case irr::EET_GUI_EVENT: {
		...
		case irr::gui::EGET_CHECKBOX_CHANGED: {
			...
			case CHECKBOX_DRAW_HOLOGRAMS: {
				gGameConfig->draw_holograms = mainGame->gSettings.chkDrawHolograms->isChecked();
				return true;
			}
			case CHECKBOX_DRAW_CARD_ANIMATIONS: {
				gGameConfig->draw_card_animations = mainGame->gSettings.chkDrawCardAnimations->isChecked();
				return true;
			}
...
void ClientField::ShowPileDisplayCards(int location, int player) {
	...
	case LOCATION_SKILL:
		loc_id = 1077;
		display_cards.assign(skill_cards[player].crbegin(), skill_cards[player].crend());
		break;
