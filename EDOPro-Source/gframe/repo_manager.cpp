...
void RepoManager::LoadRepositoriesFromJson(const nlohmann::json& configs) {
	...
	if(cit != configs.end() && cit->is_array()) {
		for(auto& obj : *cit) {
			...
			JSON_SET_IF_VALID(lflist_prefix, string, std::string);
			JSON_SET_IF_VALID(script_path, string, std::string);
			JSON_SET_IF_VALID(pics_path, string, std::string);
			JSON_SET_IF_VALID(shallow_clone, boolean, bool);
			JSON_SET_IF_VALID(update_interval_hours, number_integer, int);
...
void RepoManager::CloneOrUpdateTask() {
	...
	while(fetchReturnValue != -1) {
		...
		try {
			...
			const bool shallow_repo = _repo.shallow_clone;
			const auto update_marker = epro::format("{}/.edopro_last_update_check", path);
			auto GetNow = []() {
				return std::chrono::system_clock::to_time_t(std::chrono::system_clock::now());
			};
			auto ShouldSkipUpdate = [&]() {
				if(_repo.update_interval_hours <= 0)
					return false;
				std::ifstream input(update_marker);
				std::time_t last_check{};
				if(!(input >> last_check))
					return false;
				const auto elapsed = std::difftime(GetNow(), last_check);
				return elapsed >= 0.0 && elapsed < static_cast<double>(_repo.update_interval_hours) * 3600.0;
			};
			auto SaveUpdateCheck = [&]() {
				std::ofstream output(update_marker, std::ios::trunc);
				if(output)
					output << GetNow();
			};
			auto CloneRepo = [&]() {
				...
				try {
					...
					ErrorLog("Warning occurred while reading full history for repo {}: {}", url, history.warning);
				}
				SaveUpdateCheck();
			};
			if(DoesRepoExist(path.data())) {
				bool rebuild_repo = false;
				{
					...
					if(_repo.should_update) {
						try {
							if(!ShouldSkipUpdate()) {
								...
								if(!shallow_repo) {
									try {
										...
										history.partial_history.clear();
										history.warning = e.what();
										ErrorLog("Warning occurred while reading update history for repo {}: {}", url, history.warning);
									}
								}
							...
							history.warning = e.what();
								if(shallow_repo) {
									rebuild_repo = true;
									ErrorLog("Warning occurred in repo {}: {}. Rebuilding repository.", url, history.warning);
								} else {
									ErrorLog("Warning occurred in repo {}: {}", url, history.warning);
								}
						}
					}
					if(!rebuild_repo && !shallow_repo) {
