...
#if EDOPRO_WINDOWS
static inline epro::path_string GetCoreLibraryName() {
	return ygo::Utils::GetFileName(ygo::Utils::GetExePath(), true) == EPRO_TEXT("EDOProMod.exe") ? EPRO_TEXT("ocgmod.dll") : CORENAME;
}
static inline void* OpenLibrary(epro::path_stringview path) {
	return LoadLibrary(epro::format(EPRO_TEXT("{}{}"), path, GetCoreLibraryName()).data());
}
